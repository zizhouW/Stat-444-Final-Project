...
data.rf <- randomForest(log(Salary) ~
                        PTS + REB + AST + TOV + STL + BLK + Team + AGE + FGM + TPM + FTM + PF + PLUSMINUS + Position + Country,
                      data = data,
                      importance = TRUE,
                      subset = id.train,
                      mtry = 3)
...
data.rfcv.8 <- randomForest(log(Salary) ~
                       Team + FGM + PTS + AGE + TOV + PF + REB + FTM,
                      data = data,
                      importance = TRUE)
trainx <- data[, get.explanatory_varnames(data.rfcv.8)]
# Five fold cross-validataion
data.rfcv <- rfcv(trainx = trainx, trainy = trainy, cv.fold = 5)
# We can plot the results
with(data.rfcv, plot(n.var, error.cv, pch = 19, type="b", col="blue"))
